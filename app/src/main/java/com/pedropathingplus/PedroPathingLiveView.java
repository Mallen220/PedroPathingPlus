 ...
 * It allows the robot to communicate its position and heading to an external visualizer tool
...
public class PedroPathingLiveView {
     ...
     * This allows hot-swapping the provider between OpModes without restarting the server.
     */
    ...
    /**
     * The interval in milliseconds between telemetry updates sent to connected clients (50ms).
     */
    ...
    /**
     * Retrieves the singleton instance of {@code PedroPathingLiveView}.
     *
     ...
     * <p>
     * This convenience method extracts the pose supplier from the provided {@code Follower}.
     * Call this in your OpMode's {@code init()} method.
     * </p>
     *
     * @param follower The {@link Follower} instance to track, or {@code null} to disable tracking.
     */
     ...
     * <p>
     * Use this method if you are not using a standard {@code Follower} or need to provide
     * pose data from a different source (e.g., simulation or raw odometry).
     * </p>
     *
     * @param poseProvider A {@link Supplier} that returns the current robot {@link Pose}.
     */
     ...
     * <p>
     * It is crucial to call this in your OpMode's {@code stop()} method to release references
     * to the {@code Follower} (and thus hardware objects), preventing potential memory leaks
     ...
     * <p>
     * This method is idempotent; calling it multiple times has no effect if the server is already active.
     * The server thread is set as a daemon thread, so it will not prevent the JVM (or app) from shutting down.
     * </p>
     */
     ...
     * <p>
     * This is typically not needed if you want the server to persist across multiple OpModes.
     ...
     * <p>
     * Listens for incoming client connections on the specified port. When a client connects,
     ...
     * <p>
     * Continuously sends the current robot pose (as JSON) to the client at the defined update interval.
     * </p>
     *
